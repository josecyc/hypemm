"""Correlation stability analysis for stat arb pairs."""
...
def run_correlation_analysis(
    ...
    """Run full correlation analysis. Returns verdict string."""
    ...
    # Gate assessment
    all_high = all(float(str(r["high_pct"])) >= GATE2_MIN_HIGH_CORR_PCT for r in regime_results)
    max_breakdown = 0
    for bds_list in all_breakdowns.values():
        for bd in bds_list:
            dur = int(str(bd["duration_hours"]))
            if dur > max_breakdown:
                max_breakdown = dur

    verdict = "PASS" if (all_high and max_breakdown <= GATE2_MAX_BREAKDOWN_HOURS) else "FAIL"
    logger.info("Correlation analysis verdict: %s", verdict)
