...
def test_concurrent_unrealized_warn_then_halt(engine, risk_cfg):
    ...
    report = compute_risk_report(engine, {PAIR.label: sig}, [], risk_cfg, NOTIONAL, now_ms=NOW_MS)
...
def test_time_stop_warns_when_too_many(engine, risk_cfg):
    ...
    trades = [
        _trade(net=10, exit_offset_hours=i + 100, reason=ExitReason.TIME_STOP) for i in range(4)
    ]
...
def test_time_stop_ok_when_under(engine, risk_cfg):
    trades = [
        _trade(net=10, exit_offset_hours=i + 100, reason=ExitReason.TIME_STOP) for i in range(2)
    ]
